...
- [ ] 1.1 Re-key `src/components/features/notification/typeIcon.ts` to the real
      `NotificationType` enum (8 entries, phosphor icons); drop the import from the mock hook
- [ ] 1.2 Extract the base64url global-id encoder from
...
- [ ] 1.3 Render the shared type icon on bell popover rows (no other bell behavior change)

## 2. Polling strategy (delivery mechanism)

- [ ] 2.1 Update `useQueryMyNotificationsSwr` options: keep `refreshInterval: 60_000`, add
...
- [ ] 2.2 Add `useQueryMyNotificationsInfiniteSwr(unreadOnly)` (`useSWRInfinite`,
...
- [ ] 3.1 Rewrite `src/components/features/notification/NotificationCenter/index.tsx` onto
...
- [ ] 3.2 Wire all/unread filter to `unreadOnly` (server-side) with pagination reset on
      filter switch
- [ ] 3.3 Infinite scroll via `InfiniteScrollSentinel` with stop-at-last-page guard
- [ ] 3.4 Mark single read on row activation (optimistic + `mutateMarkNotificationAsRead`)
...
- [ ] 3.5 Row click: resolve target via `queryResolveRoute` (shared encoder) and push
      locale-prefixed path; targetless rows mark-read only; rows are keyboard-activatable
      buttons with accessible names
- [ ] 3.6 Delete the mock `hooks/useQueryNotificationsSwr` and any now-dead mock data/keys
      (grep importers first); update i18n `notificationCenter` keys in `messages/vi.json` +
      `messages/en.json` as needed

## 4. Notification preferences

- [ ] 4.1 Add mock GraphQL pair `myNotificationPreferences` /
      `updateMyNotificationPreferences` (`{ mutedTypes, muteAll }`, standard envelope with
      nullable `data`) in the FE mock API layer + types
- [ ] 4.2 Add SWR query hook + mutation hook for preferences (auth-gated)
- [ ] 4.3 Build the preferences surface opened from the center header: per-type toggles
...
- [ ] 4.4 Apply the muted-type filter to bell list, center list, and badge count
      (page-window approximation per design D5); mute-all hides badge and shows the muted
      hint in both lists
- [ ] 4.5 Add i18n keys `notifications.preferences.*` (labels per type, mute-all, push
