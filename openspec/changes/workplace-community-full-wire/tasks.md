...
- [x] 16.5 Thống kê môn (`SubjectStatistics`) và định hướng nghề theo môn (`SubjectCareer`) — ~~vẫn là dữ liệu mẫu, chưa có endpoint tương ứng~~ → **ĐÃ GIẢI PHÓNG 2026-07-25, cả hai chạy dữ liệu thật, 0 mock còn lại**:
      - `useQuerySubjectStatsSwr` đọc `GET /subjects/{code}/statistics` (`StatisticsView`): 4 số đo (tỉ lệ hoàn thành · thành viên · bài đăng · tài nguyên) + 4 bảng (học viên xuất sắc · đóng góp nhiều nhất · tài nguyên phổ biến · bảng xếp hạng). Payload chỉ mang `userId`/`resourceId` thô nên danh tính được **join** từ `GET /subjects/{code}/members` và `GET /resources/{id}`; **không bịa tên** — không khớp thì hiện `statistics.unknownMember` / `statistics.unknownResource` theo short-id và bỏ luôn link hồ sơ. `completionRate = null` → `statistics.notComputed` (KHÔNG hiện 0%). Dòng của chính người xem gắn chip `statistics.you`.
      - `useQuerySubjectCareerSwr` đọc module career thật (`/career/skills` + `/career/skills/me` + `/career/roadmaps[/{slug}]` + `/career/roadmaps/me` + `/career/opportunities` + `/career/applications/me` + `/career/recommendations/me`) và lấy `careerBridge.relatedSkills` từ workspace aggregate của môn. Lộ trình/tin tuyển dụng **liên quan môn** được đánh dấu (`career.related`, `career.roadmapMatched`); khi không có gì gắn với môn thì đổi hint sang bản "gợi ý chung toàn nền tảng" thay vì để trống. Hành động thật, optimistic + rollback: `POST /career/roadmaps/{slug}/enroll` và `POST /career/opportunities/{id}/apply`.
