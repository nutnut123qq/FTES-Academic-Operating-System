...
- [ ] 5.5 Kiểm bằng tay: đăng nhập rồi vào `/`, `/vi`, `/en` — VẪN sang `/dashboard` (góp ý #23
      còn nguyên hiệu lực)
- [ ] 5.6 Kiểm bằng tay: bấm logo navbar khi đã đăng nhập → `/dashboard`; đăng xuất rồi bấm lại
      → `/home` và ở lại đó
...
- [ ] 6.3 CÒN TREO — chớp landing ở locale root: người đã đăng nhập vào `/` vẫn thấy landing một
      ...
      xuống 1
- [ ] 6.4 CÒN TREO — cửa sổ hydration của Logo: người đã đăng nhập bấm logo trước khi phiên ngã
      ngũ sẽ về `/home` chứ không về `/dashboard`. Chấp nhận có chủ đích (mục 2.3). Hết hẳn thì
      cần tín hiệu phiên đọc được trước paint — cùng hạng mục với 6.3
