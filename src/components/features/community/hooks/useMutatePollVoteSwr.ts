 ...
 * with the option's genuine UUID. On success the poll's cache key is revalidated so
 * `myOptionId`/`voteCount` reflect the server tally.
 *
...
export const useMutatePollVoteSwr = () => {
    ...
    return useCallback(
        async (postId: string, optionId: string): Promise<boolean> => {
            if (!(await requireAuthAsync("auth.context.vote"))) {
                return false
            }
            await votePoll(postId, { optionId })
            await mutate(pollSwrKey(postId))
            return true
        },
