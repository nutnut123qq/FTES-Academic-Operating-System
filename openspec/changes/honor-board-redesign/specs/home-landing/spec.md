...
name and achievement lines SHALL be DOM text (name rendered with a gold-gradient
text treatment via the `warning` token — never baked into an image); each card
shows the portrait, the name, one highlight metric as the dominant element, and
the achievement lines. Gold (`warning`) SHALL be used as an accent only (name,
metric, ring, glow) on translucent glass cards over a subtle ambient-gold section
background, working in both light and dark themes.
