...
import { Switch, Typography } from "@heroui/react"
...
import { PreferencesSurfaceSkeleton } from "./skeleton"

/**
 * PreferencesSurface — the notification-preferences surface, mounted in TWO places:
 * the settings section (`/profile/settings/notifications`) and behind the gear on the
 * `/notifications` centre. Both read {@link useGetNotificationPreferencesSwr}, i.e. the
 * SAME SWR key, so the two entry points can never disagree.
 *
 * Renders a master "mute all" switch, one toggle per real {@link NotificationType}
 * (disabled while mute-all is on), and a reserved-but-disabled "browser push — coming
 * soon" row (Web Push is deferred: no service worker, no permission prompt, no
 * subscription — see the change design D6). Preferences are read + written through the
 * real BE REST API (`GET/PUT /api/v1/notifications/preferences`, IN_APP column of the
 * matrix); every write is OPTIMISTIC — the shared SWR cache flips immediately (bell,
 * center and badge react at once) and rolls back with an error toast when the PUT fails.
 */
export const PreferencesSurface = () => {
    ...
    return (
        <AsyncContent
            ...
            skeleton={<PreferencesSurfaceSkeleton />}
        >
            <section
