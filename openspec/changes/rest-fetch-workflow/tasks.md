## 1. Scaffold Workflow REST module

- [x] 1.1 Create `src/modules/api/rest/workflow/` directory with `types.ts`, `workflow.ts`, and `index.ts`.
- [x] 1.2 Define all request/response types in `types.ts` with the `Workflow*` prefix.
- [x] 1.3 Implement REST call functions in `workflow.ts` using `restRequest`.
