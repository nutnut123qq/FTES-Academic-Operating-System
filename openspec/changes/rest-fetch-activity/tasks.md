## 1. Scaffold Activity REST module

- [x] 1.1 Create `src/modules/api/rest/activity/` directory with `types.ts`, `activity.ts`, and `index.ts`.
- [x] 1.2 Define all request/response types in `types.ts` with the `Activity*` prefix.
- [x] 1.3 Implement REST call functions in `activity.ts` using `restRequest`.
