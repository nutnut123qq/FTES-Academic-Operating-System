...
- [x] 2.2 **Tư thế LÓ tới PHẦN THÂN, nép hẳn vào góc phải dưới** (khớp ảnh mẫu chủ sản phẩm): `w-24` (96px) mobile / `sm:w-36` (144px), đẩy chéo qua mép để **giữ lại 68% chiều cao + 90% bề ngang** — mép cắt rơi ngay dưới ngực nên còn đủ đầu + tay vẫy + thân áo có chữ FTES. Độ đẩy NUỐT luôn phần lề của vỏ ngoài (16px mobile / 24px desktop) nên nhân vật nằm sát góc trong khi PANEL vẫn giữ lề với mép màn. **KHÔNG nghiêng** — `rotate` đã thử và sai: nghiêng làm thân văng khỏi góc, cùng một độ đẩy thì chỉ còn cái đầu ló vào. Transform đặt trên NÚT (vùng bấm đi theo hình), không đặt trên ảnh (ảnh giữ `.mascot-float`). `-mt` ĐỐI XỨNG với `translate-y`. `z-40` (dưới modal `z-50`)
...
- [x] 7.5c Số đo trình duyệt thật khớp đúng bản dựng: 412×915 → vẽ 96px, cắt 10px phải / 43px đáy, thấy 86×92; 1280×720 → vẽ 144px, cắt 65px đáy, thấy 144×137; `rotate: none`; **không sinh cuộn ngang**; panel nằm trọn trong màn, cách đỉnh thấy được 2px
