...
- **[Amendment — hướng mới của product owner]** Header đã được ship theo bản gốc; đây là delta chỉnh hướng:
  - **Community ▾ nhấn mạnh Group + Event khi hover** — module Community giữ nguyên dropdown (feed/groups/events/chat) nhưng hover PHẢI lộ Groups + Events (đúng câu "hover vào Community thì hiện ra Group, Event").
  - **Gỡ các mục "khám phá" khỏi header modules**: bỏ `ai` (AI Hub) khỏi Workplace ▾ và `recommendations` khỏi Course ▾. Sau delta: Workplace ▾ = subjects, resources, challenges, leaderboard, workflow, analytics, career; Course ▾ = catalog, marketplace.
  - **Discovery shortcuts KHÔNG còn là header module** — global AI hub, For You, Recommendations, Trending chuyển sang **profile/avatar popup** (owner: change `profile-avatar-hub`, section "Khám phá"). Route `/ai` và `/recommendations` vẫn hợp lệ và vẫn reachable qua popup → KHÔNG route nào mồ côi.
