...
- [ ] 1.1 Create `public/subjects/` and copy existing repo artwork to per-code files:
...
- [ ] 1.2 Extend `Subject` in `src/components/features/subject/hooks/useQuerySubjectSwr.ts`
...
- [ ] 1.3 Populate `useQuerySubjectsSwr.ts` mock rows with `imageUrl` per code; set
...
- [ ] 2.1 In `SubjectCatalog/index.tsx`, add the 16:9 `next/image` cover thumbnail
...
- [ ] 2.2 Handle `imageUrl: null` and image `onError` → render today's image-less card
      (no empty box, no broken glyph)
- [ ] 2.3 Add the loading skeleton: gate the grid on `isLoading || error` and render
...
- [ ] 3.1 In `SubjectWorkspaceShell/index.tsx`, render the subject image in the `size-11`
...
- [ ] 4.1 Confirm `subjects.*` i18n keys are untouched and `vi`/`en` messages stay
      key-parallel (no new strings expected)
- [ ] 4.2 `npm run build` (webpack) green
- [ ] 4.3 `npx tsc --noEmit` clean
