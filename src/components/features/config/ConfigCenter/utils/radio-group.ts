...
/**
 * Roving-focus keyboard handler for the hand-rolled radiogroups (flag status).
 * Attached to the `role="radiogroup"` container: arrow keys move focus to the
 * previous/next enabled `role="radio"` child and select it (radios select on
 * focus), wrapping at both ends. Mirrors the AppearanceModal helper.
