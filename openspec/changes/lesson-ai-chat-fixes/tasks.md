...
- [ ] 1.3 Unit test (RTL): có selection → fetch body chứa passage + context block, bubble chỉ hiện display; không selection → body = raw — CHẶN: worktree ai-fe KHÔNG có test runner (vitest/jest/RTL đều chưa cài, không có script `test`, 0 file test). Xem backlog.
...
- [ ] 2.6 Unit test: default từ catalog, model ride body, remount giữ model, degrade khi catalog lỗi, caption render — CHẶN: worktree ai-fe KHÔNG có test runner (0 vitest/jest/RTL, không script `test`). Xem backlog (chung blocker với Feature A 1.3)
...
- [ ] 3.5 Unit test: định vị 3 nhánh (phải/trái/dưới), clamp, close handlers, intent không bị reset lúc mount; mobile không render panel — CHẶN: worktree ai-fe KHÔNG có test runner (chung blocker Feature A 1.3 / B 2.6). Xem backlog
