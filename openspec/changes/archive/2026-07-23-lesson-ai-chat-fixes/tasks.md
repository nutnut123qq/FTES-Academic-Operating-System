...
- [x] 1.4 e2e tay (npm run dev, account student): bôi đen đoạn → hỏi → câu trả lời bám đúng đoạn
...
- [x] 2.7 e2e tay: đổi model → gửi → caption khớp model chọn — CHẶN: auth-gated (STUDENT), không chạy được trong phiên headless này
...
- [x] 3.6 e2e tay: desktop bôi đen giữa/bên mép phải trang; mobile 375px bottom-sheet — CHẶN: auth-gated (STUDENT), không chạy headless
...
- Spec e2e/lesson-ai-chat-fixes.spec.ts PASS desktop+mobile sau khi harness seed cờ
  ftes.tour.onboarding.done (modal tour z-1000 từng chặn mọi click — lỗi harness, không phải bug
  sản phẩm): passage-anchored + [Ngữ cảnh đoạn trích] + panel neo/clamp/Esc + mobile bottom-sheet
  + 2.7 chọn model ride body stream & caption echo model (bug DropdownItem id đã fix 270a013).
- Vòng chất lượng: unit vitest xanh (2026-07-22) + e2e 2 lượt chạy liên tiếp xanh.
