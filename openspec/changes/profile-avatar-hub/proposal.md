...
- **Account dropdown v2 (logged in)**: header block = avatar + name wrapped in a level ring; new stats row of compact chips 🔥 streak · 🏆 rank · ⚡ XP linking to profile/leaderboard; then existing menu links (Profile, Saved, Settings), theme switch, logout. Skeleton chips while stats load. Guest dropdown unchanged.
- **Profile completion**:
...
- `account-menu-gamification`: logged-in avatar dropdown v2 — level-ring header, stats chip row with loading/navigation behavior, unchanged guest state.
...
- Affected components: `src/components/features/navbar/Navbar/AccountMenuDropdown/` (extended), `ProfileShell` (identity card + Progress/Portfolio/Community tabs filled).
