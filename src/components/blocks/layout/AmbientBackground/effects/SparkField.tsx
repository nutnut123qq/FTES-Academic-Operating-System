...
export interface SparkFieldProps {
     ...
     * `"fall"` = elongated meteor streaks shooting diagonally (top-left →
     * bottom-right) with a comet tail (the app default).
     */
...
/**
 * The app's own ambient spark field (the `ember` effect): a faint accent glow
 * hugging one viewport edge plus embers drifting slowly upward
 * (`direction="rise"`) or a meteor shower of comet-tailed streaks shooting
 * diagonally top-left → bottom-right (`direction="fall"`, default). `speed`
 * scales every spark's duration.
 *
 * Colours come from the `--accent` token so it tracks the selected accent +
 * light/dark automatically; the keyframes (`emberRise`, `meteorFall`) +
 * reduced-motion guard live in `globals.css`. Sparks are laid out
 * deterministically (seeded by index) so server + client markup match — no
 * hydration mismatch and no `Math.random` at render.
 *
 * @param props - {@link SparkFieldProps}
 */
